...
CLUSTERING = {'kmeans' : lambda df, k: KMeans(n_clusters = k, random_state=42).fit(df).labels_,
...
class SCluster:
    ...
    def fit(self,data):
        ...
        self.size = self.n
        self.df = data
        sil_dict = {'optimal_cluster' : 0, 'silhouette' : 0}
        input = []
        silhouette = []
        for i in range(self.org, self.lim , self.stp):
            ...
            input.append(i)
            silhouette.append(round(silho, 2))
            self.scores[silho] = label
            self.max = silho if self.max<silho else self.max
            #print("cluster kind: " + str(self.type) + ", input value = " + str(i) + ", silhouette = " + str(round(silho, 2)))
        max_value = max(silhouette)
        num_cluster = input[silhouette.index(max_value)]
        sil_dict['optimal_cluster'] = num_cluster
        sil_dict['silhouette'] = max_value
        self.result = sil_dict
